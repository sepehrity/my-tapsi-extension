...
const Charts = ({
  data: { _hours, _days, _weeks, _months, _methods, _years, _cars },
}: Props) => {
  ...
  }, [_hours, _days, _weeks, _months, _methods, _years]);

  const CarCharts: CountPriceObject[] = useMemo(() => {
    return getCarsChunks(getSortedPattern(_cars, [], 'count'));
  }, [_cars]);

  return (
    <>
      {Object.keys(BarCharts).map((type, index) => {
        const chart_data: CountPriceObject = BarCharts[type];
        return (
          <BarChart
            key={type}
            color={colors[index]}
